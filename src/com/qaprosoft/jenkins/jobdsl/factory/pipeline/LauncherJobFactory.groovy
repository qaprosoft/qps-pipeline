	...
	def create() {
		...
		pipelineJob.with {
			...
			parameters {
				...
				configure addHiddenParameter('suite', 'TestNG suite file name (without ".xml" extension)', "api")
				configure addHiddenParameter('overrideFields', 'Custom parameters to run job with' , "-Dplatform=API -Dthread_count=5")
